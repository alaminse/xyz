                ...
                <div class="x_panel">
                    <div class="x_title d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            ...
                            <a href="{{ route('admin.mcqs.index') }}" class="btn btn-warning btn-sm text-white">
                    ...
                    <div class="x_content">
                            ...
                            <div class="col-md-3">
                                <div class="card border-0 shadow-sm">
                                    <div class="card-body">
                                        ...
                                        <h6 class="mb-0 font-weight-bold">{{ $mcq->chapter?->name ?? 'N/A' }}</h6>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-3">
                                <div class="card border-0 shadow-sm">
                                    <div class="card-body">
                                        ...
                                        <h6 class="mb-0 font-weight-bold">{{ $mcq->lesson?->name ?? 'N/A' }}</h6>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-3">
                                <div class="card border-0 shadow-sm">
                                    <div class="card-body">
                                        ...
                                        <h6 class="mb-0 font-weight-bold">{{ $mcq->questions->count() }}</h6>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card shadow-sm">
                            ...
                            <div class="card-body">
                                @forelse($mcq->questions as $index => $question)
                                    <div class="card question-card mb-3">

                                        <!-- HEADER -->
                                        <div class="card-header bg-light d-flex justify-content-between align-items-center"
                                            data-toggle="collapse" data-target="#questionCollapse{{ $question->id }}"
                                            ...
                                            <div class="d-flex align-items-center">
                                                <button class="btn btn-sm btn-primary edit-question mr-1"
                                                    data-id="{{ $question->id }}" onclick="event.stopPropagation();">
                                        ...
                                        <!-- BODY -->
                                        <div id="questionCollapse{{ $question->id }}" class="collapse">
                                            <div class="card-body">
                                                    ...
                                                    <tbody>
                                                        @for ($i = 1; $i <= 5; $i++)
                                                            <tr>
                                                                ...
                                                                <td>{{ $question->{"option{$i}"} }}</td>
                                                                <td class="text-center">
                                                                    <span
                                                                        class="badge {{ $question->{"answer{$i}"} ? 'badge-success' : 'badge-danger' }}">
                                                                        {{ $question->{"answer{$i}"} ? 'True' : 'False' }}
                                                                    </span>
                                                                </td>
                                                            </tr>
                                                        @endfor
                                ...
                                <div class="row">
                                    <div class="col-md-9">
                                        <input type="text" name="option{{ $i }}"
                                            id="option{{ $i }}" class="form-control" required
                                            maxlength="500">
                                    </div>
                                    <div class="col-md-3 text-right">
                                        <div class="form-check mt-2">
                                            <input type="checkbox" name="answer{{ $i }}"
                                                id="answer{{ $i }}" class="form-check-input" value="1">
                                            <label class="form-check-label" for="answer{{ $i }}">True</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            const mcqId = {{ $mcq->id }};
            const mcqChapterId = {{ $mcq->chapter_id ?? 'null' }};
            const mcqLessonId = {{ $mcq->lesson_id ?? 'null' }};
            const mcqCourseIds = @json($mcq->courses->pluck('id')->toArray());
