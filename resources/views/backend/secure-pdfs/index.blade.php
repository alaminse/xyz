            ...
            <ul class="nav navbar-right panel_toolbox">
                <li>
                    <a class="btn btn-sm btn-success text-light"
                       href="{{ route('admin.secure-pdfs.create') }}">
                ...
                <div class="col-12">
                    <div class="d-flex flex-wrap">
                        @foreach ($courses as $key => $course)
                            <button
                                class="btn btn-outline-primary mr-2 mb-2 nav-link {{ $key == 0 ? 'active' : '' }}"
                                id="{{ $course->slug }}-tab"
                                data-toggle="tab"
...
function getPdfs(slug) {
    $.ajax({
        url: '{{ route("admin.secure-pdfs.data") }}',
        data: { slug: slug },
        method: 'GET',
