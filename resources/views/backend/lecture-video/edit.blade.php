...
@section('title', 'Edit Lecture Video')
        ...
        <div class="x_panel">
            <div class="x_title">
                ...
                <ul class="nav navbar-right panel_toolbox">
                    <li><a class="btn btn-warning text-white" href="{{ route('admin.lecturevideos.index') }}">
            ...
            <div class="x_content">
                <form action="{{ route('admin.lecturevideos.update', $video->id) }}" method="POST" id="videoForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                            ...
                            <select name="course_ids[]" id="courseSelect" class="form-control select2" multiple required>
                                @foreach ($courses as $course)
                                    <option value="{{ $course->id }}"
                                        {{ collect(old('course_ids', $video->courses->pluck('id')))->contains($course->id) ? 'selected' : '' }}>
                                        {{ $course->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <div class="mt-2">
                                <label class="checkbox-inline">
                                    <input type="checkbox" name="isPaid" value="1"
                                        {{ old('isPaid', $video->isPaid) ? 'checked' : '' }}>
                        ...
                        <div class="col-sm-12 col-md-6 mb-3">
                            ...
                            <select name="status" class="form-control">
                                ...
                                <option value="2" {{ old('status', $video->status) == 2 ? 'selected' : '' }}>Inactive</option>
                            </select>
                        </div>
                        ...
                        <div class="col-sm-12 mb-3">
                            <div class="current-video-info">
                                ...
                                @if($video->youtube_link)
                                    ...
                                    <p class="mb-0 mt-2"><small>URL: {{ $video->youtube_link }}</small></p>
                                @elseif($video->uploaded_link)
                                    <span class="badge badge-primary">
                                        <i class="fa fa-cloud"></i> Bunny.net Video
                                    </span>
                                    <p class="mb-0 mt-2"><small>Video ID: <code>{{ $video->uploaded_link }}</code></small></p>
                                @else
                    ...
                    <div class="form-group">
                        <div class="col-12">
                            ...
                            <a href="{{ route('admin.lecturevideos.show', $video->id) }}" class="btn btn-info">
                                <i class="fa fa-eye"></i> View Details
                            </a>
                            <a href="{{ route('admin.lecturevideos.index') }}" class="btn btn-secondary">
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            const courseIds = @json(old('course_ids', $video->courses->pluck('id')));
            const chapterId = '{{ old('chapter_id', $video->chapter_id) }}';
            const lessonId = '{{ old('lesson_id', $video->lesson_id) }}';
