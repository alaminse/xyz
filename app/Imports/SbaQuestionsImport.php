...
class SbaQuestionsImport implements OnEachRow, WithHeadingRow
{
    ...
    public function onRow(Row $row)
    {
        ...
        $correctOption = trim($data['correct_option'] ?? '');
        ...
        if ($correctOption === '') {
            $this->addSkip($rowNumber, 'Missing correct_option.');
            return;
        }
        ...
        if (isset($this->sbaCache[$cacheKey])) {
            $sba = $this->sbaCache[$cacheKey];
        } else {
            $sba = Sba::where('chapter_id', $chapter->id)
                ->where('lesson_id', $lesson->id)
                ->first();

            if (! $sba) {
                if ($courseName === '') {
                    $this->addSkip($rowNumber, "No SBA set exists for '{$chapterName}' / '{$lessonName}', and no course_name given to auto-create one.");
                    return;
                }
                ...
                if ($courses->isEmpty()) {
                    $this->addSkip($rowNumber, "Course(s) '{$courseName}' not found. Cannot auto-create SBA set.");
                    return;
                }
                ...
                $sba->courses()->sync($courses->pluck('id'));

                $this->createdSets++;
            }

            $this->sbaCache[$cacheKey] = $sba;
        }
